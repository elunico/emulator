...
struct cpu {
 ...
 private:
  ...
  u32 *register_decode(u32 reg_index) {
    ...
    if (reg_index < 1 || reg_index > regs.size()) malformed();
    return regs[reg_index];
  }

  u32 literal_decode_24(u32 instruction) {
    u32 literal = instruction & 0xffffff;
    return literal;
  }

  u32 literal_decode_8(u32 instruction) {
    u32 literal = instruction & 0xff;
    return literal;
  }
    ...
    *metaout << "tick" << std::endl;
    ...
    switch (opcode) {
      case opcodes::LD_IM_A: {
        *metaout << "Loading into a " << literal_decode_24(instruction)
                 << std::endl;
        a = literal_decode_24(instruction);
        ctrl_check(&a);
      } break;
      case opcodes::LD_IM_B: {
        *metaout << "Loading into b " << literal_decode_24(instruction)
                 << std::endl;
        b = literal_decode_24(instruction);
        ctrl_check(&b);
      } break;
      case opcodes::INC_A: {
        a++;
      ...
      case opcodes::JMP_WITH_OFFSET: {
        *metaout << " JUMP " << std::endl;
        u32 addr = literal_decode_24(instruction);
        pc -= addr;
