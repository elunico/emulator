...
private:
  ...
  u32 ctrl_get(u32 bitmask) const { return ctrl & bitmask; }
  ...
  u32 fetch(u32 const &r) const {
    u32 instr = (ram[r] << 24) | (ram[r + 1] << 16) | (ram[r + 2] << 8) |
                (ram[r + 3] << 0);
    return instr;
  }

  std::tuple<u32 *, u32 *, u32 *> register_decode_dss(u32 instruction) const {
  ...
  std::pair<u32 *, u32 *> register_decode_dsi(u32 instruction) const {
    u32 sdb = (instruction & 0xff0000) >> 16;
    u32 srb = (instruction & 0xff00) >> 8;
  ...
  std::pair<u32 *, u32 *> register_decode_both(u32 instruction) const {
  ...
  u32 *register_decode_first(u32 instruction) const {
    u32 reg1 = instruction & 0xff;
    return register_decode(reg1);
  }

  std::vector<u32 *> const regs = {{(u32 *)&z, &a, &b, &x, &sp, &ra}};
  std::vector<f64 *> const fregs = {{NULL, &fa, &fb, &fx}};

  u32 *register_decode(u32 reg_index) const {
    *metaout << "index " << reg_index << std::endl;
    if (reg_index < 0 || reg_index > regs.size())
      invalid_registers();
    return regs[reg_index];
  }

  template <u32 N, typename Return = u32>
  Return literal_decode(u32 instruction) const noexcept {
    Return mask = (1 << N) - 1;
  ...
  [[noreturn]] void invalid_registers() const {
  ...
  void tick() {
    *metaout << "tick" << std::endl;
    ...
    case opcodes::TEST_NEQ: {
      ...
      *metaout << "Value lhs = " << *lhs << " and rhs = " << *rhs << std::endl;
